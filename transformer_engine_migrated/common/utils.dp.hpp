...
/*
DPCT1011:4: The tool detected overloaded operators for built-in vector types,
...
/*
DPCT1011:5: The tool detected overloaded operators for built-in vector types,
which may conflict with the SYCL 2020 standard operators (see 4.14.2.1 Vec
interface). The tool inserted a namespace to avoid the conflict. Use SYCL 2020
standard operators instead.
*/
...
template <> struct TypeToVec2<sycl::ext::oneapi::bfloat16> {
...
template <>
struct Converter<sycl::float2, sycl::marray<sycl::ext::oneapi::bfloat16, 2>> {
    static inline sycl::marray<sycl::ext::oneapi::bfloat16, 2>
    convert(const sycl::float2 &x) {
...
struct InterCTASync {
    ...
    inline void spin_wait_(int *barrier, int step, int expected) {
      ...
      found = *((uint32_t *)(uintptr_t)barrier);
        }
    }
