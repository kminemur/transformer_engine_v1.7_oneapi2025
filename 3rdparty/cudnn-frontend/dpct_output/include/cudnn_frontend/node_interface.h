...
class INode : public ICudnn {
   ...
   private:
    ...
    extend_tensor_map_with_pass_by_value_tensors_(
        ...
        for (auto& [uid, value] : tensor_to_pass_by_value) {
            if (sycl::half* half_value_ptr = std::get_if<sycl::half>(&value)) {
                tensor_to_pointer_map.emplace(uid, half_value_ptr);
            } else if (sycl::ext::oneapi::bfloat16* nv_bfloat16_value_ptr =
                           std::get_if<sycl::ext::oneapi::bfloat16>(&value)) {
   ...
   public:
    ...
    error_t
    build_operation_graph(dpct::dnnl::engine_ext handle) {
    ...
    error_t
    execute_plan_at_index(dpct::dnnl::engine_ext handle,
                          std::unordered_map<std::shared_ptr<Tensor_attributes>, void*>& tensor_to_pointer_map,
                          void* workspace,
    ...
    error_t
    deserialize(dpct::dnnl::engine_ext handle, std::vector<uint8_t> const& data) {
