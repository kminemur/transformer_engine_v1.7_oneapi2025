    ...
    expand_and_infer_properties_node() override final {
        ...
        if (dropout_present) {
            ...
            if (get_backend_version() < 8903) {
                sycl::half dropout_scale_value =
                    sycl::vec<float, 1>(1.0f / (1.0f - attributes.dropout_probability.value()))
...
#undef CUDNN_FE_SDPA_VALIDATE_DIM_STRIDE
    ...
    error_t
    expand_and_infer_properties_node() override final try {
        ...
        /*
        DPCT1005:766: The SYCL device version is different from CUDA Compute Compatibility. You may need to rewrite this
        code.
        */
        if ((cudnn_frontend::get_backend_version() >= 8905 && prop.get_major_version() >= 9) ||
