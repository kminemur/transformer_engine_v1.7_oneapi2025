...
inline error_t
execute(dpct::dnnl::engine_ext handle,
        ExecutionPlan* plan,
...
query_heuristics(std::vector<std::shared_ptr<OperationGraph_v8>> const& operation_graphs,
    ...
    for (auto const& operation_graph : operation_graphs) {
        ...
        for (auto& engine_config : configs) {
            ...
            /*
            DPCT1007:718: Migration of CUDNN_BACKEND_ENGINE_DESCRIPTOR is not supported.
            */
            ...
            auto status =
                cudnn_frontend::get_attribute(engine_config->get_backend_descriptor(),
                                              /*
                                              DPCT1007:719: Migration of CUDNN_ATTR_ENGINECFG_ENGINE is not supported.
                                              */
                                              CUDNN_ATTR_ENGINECFG_ENGINE,
                                              /*
                                              DPCT1007:720: Migration of CUDNN_TYPE_BACKEND_DESCRIPTOR is not supported.
                                              */
   ...
   public:
    ...
    query_properties() {
        ...
        for (auto& engine_config : engine_configs) {
            ...
            /*
            DPCT1007:721: Migration of CUDNN_BACKEND_ENGINE_DESCRIPTOR is not supported.
            */
            ManagedOpaqueDescriptor extractedEngine   = make_shared_backend_pointer(CUDNN_BACKEND_ENGINE_DESCRIPTOR);
            cudnnBackendDescriptor_t extractedEngine_ = extractedEngine->get_backend_descriptor();
            auto status =
                cudnn_frontend::get_attribute(engine_config->get_backend_descriptor(),
                                              /*
                                              DPCT1007:722: Migration of CUDNN_ATTR_ENGINECFG_ENGINE is not supported.
                                              */
                                              CUDNN_ATTR_ENGINECFG_ENGINE,
                                              /*
                                              DPCT1007:723: Migration of CUDNN_TYPE_BACKEND_DESCRIPTOR is not supported.
                                              */
                                              CUDNN_TYPE_BACKEND_DESCRIPTOR,
                                              1,
                                              &elem_count,
                                              &extractedEngine_);
            RETURN_CUDNN_FRONTEND_ERROR_IF(
                (status != 0), error_code_t::HEURISTIC_QUERY_FAILED, "Heuristic query Engine failed.");

            status = cudnn_frontend::get_attribute(
                extractedEngine_,
                /*
                DPCT1007:724: Migration of CUDNN_ATTR_ENGINE_NUMERICAL_NOTE is not supported.
                */
                CUDNN_ATTR_ENGINE_NUMERICAL_NOTE,
                /*
                DPCT1007:725: Migration of CUDNN_TYPE_NUMERICAL_NOTE is not supported.
                */
                CUDNN_TYPE_NUMERICAL_NOTE,
                /*
                DPCT1007:726: Migration of CUDNN_NUMERICAL_NOTE_TYPE_COUNT is not supported.
                */
                CUDNN_NUMERICAL_NOTE_TYPE_COUNT,
                &elem_count,
                nullptr);
            RETURN_CUDNN_FRONTEND_ERROR_IF(
                (status != 0), error_code_t::HEURISTIC_QUERY_FAILED, "Heuristic query Numerical Note failed");

            numerics.resize(static_cast<size_t>(elem_count));
            status = cudnn_frontend::get_attribute(
                extractedEngine_,
                /*
                DPCT1007:727: Migration of CUDNN_ATTR_ENGINE_NUMERICAL_NOTE is not supported.
                */
                CUDNN_ATTR_ENGINE_NUMERICAL_NOTE,
                /*
                DPCT1007:728: Migration of CUDNN_TYPE_NUMERICAL_NOTE is not supported.
                */
                CUDNN_TYPE_NUMERICAL_NOTE,
                /*
                DPCT1007:729: Migration of CUDNN_NUMERICAL_NOTE_TYPE_COUNT is not supported.
                */
                CUDNN_NUMERICAL_NOTE_TYPE_COUNT,
                &elem_count,
                numerics.data());
            ...
            status = cudnn_frontend::get_attribute(
                extractedEngine_,
                /*
                DPCT1007:730: Migration of CUDNN_ATTR_ENGINE_BEHAVIOR_NOTE is not supported.
                */
                CUDNN_ATTR_ENGINE_BEHAVIOR_NOTE,
                /*
                DPCT1007:731: Migration of CUDNN_TYPE_BEHAVIOR_NOTE is not supported.
                */
                CUDNN_TYPE_BEHAVIOR_NOTE,
                /*
                DPCT1007:732: Migration of CUDNN_BEHAVIOR_NOTE_TYPE_COUNT is not supported.
                */
                CUDNN_BEHAVIOR_NOTE_TYPE_COUNT,
                &elem_count,
                nullptr);
            RETURN_CUDNN_FRONTEND_ERROR_IF(
                (status != 0), error_code_t::HEURISTIC_QUERY_FAILED, "Heuristic query Behavior Note failed");

            behavior.resize(static_cast<size_t>(elem_count));
            status = cudnn_frontend::get_attribute(
                extractedEngine_,
                /*
                DPCT1007:733: Migration of CUDNN_ATTR_ENGINE_BEHAVIOR_NOTE is not supported.
                */
                CUDNN_ATTR_ENGINE_BEHAVIOR_NOTE,
                /*
                DPCT1007:734: Migration of CUDNN_TYPE_BEHAVIOR_NOTE is not supported.
                */
                CUDNN_TYPE_BEHAVIOR_NOTE,
                /*
                DPCT1007:735: Migration of CUDNN_BEHAVIOR_NOTE_TYPE_COUNT is not supported.
                */
                CUDNN_BEHAVIOR_NOTE_TYPE_COUNT,
                &elem_count,
                behavior.data());
    ...
    filter_numeric_notes(std::vector<NumericalNote_t> const& notes, bool const keep) {
        for (auto& note : notes) {
            ...
            auto valid_note = (detail::convert_to_cudnn_type(note, backend_note) == 0);
    ...
    error_t
    check_support(dpct::dnnl::engine_ext handle) {
        for (auto i = 0u; i < engine_configs.size(); i++) {
            if (barred_indices[i]) {
    ...
    error_t
    build_plans(dpct::dnnl::engine_ext handle, std::string const& json) {
    ...
    error_t
    build_plan_at_index(dpct::dnnl::engine_ext handle, int64_t index) {
    ...
    error_t
    build_plans(dpct::dnnl::engine_ext handle, BuildPlanPolicy_t const policy, bool const do_multithreaded_builds) {
    ...
    error_t
    autotune(dpct::dnnl::engine_ext handle,
             std::unordered_map<int64_t, void*> const& tensor_to_pointer_map,
             void* workspace,
