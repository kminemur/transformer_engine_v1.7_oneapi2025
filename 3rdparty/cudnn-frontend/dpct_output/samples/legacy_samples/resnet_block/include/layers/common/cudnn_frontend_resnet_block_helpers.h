...
generateStrides(const int64_t *dimA, int64_t *strideA, int64_t nbDims, dpct::dnnl::memory_format_tag filterFormat) {
    if (filterFormat == dpct::dnnl::memory_format_tag::nchw) {
        strideA[nbDims - 1] = 1;
...
static constexpr dpct::library_data_t data_type         = dpct::library_data_t::real_float;
...
struct ConvParams {
    ...
    dpct::library_data_t dataType = dpct::library_data_t::real_half;
};
...
typedef struct convolution_params_ {
    ...
    dpct::library_data_t tensor_data_type;
    dpct::library_data_t compute_type;
...
make_convolution_params(convolution_params::uids_t const uid_offset,
    ...
    return {/* dim count  */ 4,
            ...
            /* dilation */ {1, 1},
            /*
            DPCT1007:903: Migration of CUDNN_DATA_FP8_E4M3 is not supported.
            */
            /* tensor data type */ CUDNN_DATA_FP8_E4M3,
            /* compute type */ dpct::library_data_t::real_float,
            /* uids */
            {
...
make_bn_fusion_params(bn_fusion_params::uids_t uid_offset,
    ...
    return {/* dim count  */ 4,
            ...
            /* has add relu */ false,
            /* has relu */ false,
            /* compute_type */ dpct::library_data_t::real_float,
            /*
            DPCT1007:904: Migration of CUDNN_DATA_FP8_E4M3 is not supported.
            */
...
make_pointwise_parameters(pointwise_parameters::uids_t uid_offset,
    ...
    return {/* dim count  */ 4,
            ...
            /*
            DPCT1007:905: Migration of CUDNN_DATA_FP8_E4M3 is not supported.
            */
            /* data_type */ CUDNN_DATA_FP8_E4M3,
            /* compute_type */ dpct::library_data_t::real_float,
            /* uids */
...
struct StatsParams {
    ...
    /*
    DPCT1007:906: Migration of CUDNN_GENSTATS_SUM_SQSUM is not supported.
    */
    cudnnGenStatsMode_t mode = CUDNN_GENSTATS_SUM_SQSUM;
    /*
    DPCT1007:907: Migration of CUDNN_BN_FINALIZE_STATISTICS_TRAINING is not supported.
    */
    cudnnBnFinalizeStatsMode_t finalizeMode = CUDNN_BN_FINALIZE_STATISTICS_TRAINING;
    dpct::library_data_t dataType           = dpct::library_data_t::real_float;
};
 ...
 */
static inline void
logErrorMessage(dpct::err1 status,
    ...
    if (descriptorType == "execution plan") {
        errorMessage = "[ERROR]: CUDNN Error in exeucting plan with name: " + descriptorName +
                       "! This error happens in the " + blockName +
                       /*
                       DPCT1009:908: SYCL uses exceptions to report errors and does not use the error codes. The call
                       was replaced by a placeholder string. You need to rewrite this code.
                       */
                       "! Double check your parameters/device pointers. CUDNN error string: " +
                       "<Placeholder string>" + " " + extraMessage;
        ...
        errorMessage = "[ERROR]: Error in creating : " + descriptorType + " with name: " + descriptorName +
                       ...
                       /*
                       DPCT1009:909: SYCL uses exceptions to report errors and does not use the error codes. The call
                       was replaced by a placeholder string. You need to rewrite this code.
                       */
                       "<Placeholder string>" +
                       " " + extraMessage;
        ...
        errorMessage = "[ERROR]: Error in creating : " + descriptorType + " with name: " + descriptorName +
                       ...
                       /*
                       DPCT1009:910: SYCL uses exceptions to report errors and does not use the error codes. The call
                       was replaced by a placeholder string. You need to rewrite this code.
                       */
                       "<Placeholder string>" +
                       " " + extraMessage;
        ...
        errorMessage = "[ERROR]: CUDNN Error in creating descriptor type: " + descriptorType +
                       " with name: " + descriptorName + "! This error happens in the " + blockName +
                       /*
                       DPCT1009:911: SYCL uses exceptions to report errors and does not use the error codes. The call
                       was replaced by a placeholder string. You need to rewrite this code.
                       */
                       "! Double check your parameters/device pointers. CUDNN error string: " +
                       "<Placeholder string>" + " " + extraMessage;
 ...
 */
static inline bool
checkErrorStatusAndLog(dpct::err1 status,
                       const std::string &blockName,
                       const std::string &descriptorType,
                       const std::string &descriptorName,
                       std::string &errorMessage,
