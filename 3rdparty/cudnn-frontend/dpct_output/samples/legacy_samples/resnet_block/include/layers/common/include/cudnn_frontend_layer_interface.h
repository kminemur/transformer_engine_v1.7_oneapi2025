...
class ProblemDescBuilder {
   public:
     ...
     */
    void
    configureMathPrec(const dpct::library_data_t &mathPrec) {
        mathPrec_ = mathPrec;
    };
   ...
   private:
    ...
    dpct::library_data_t mathPrec_ = dpct::library_data_t::real_float;
    // Pointwise mode defaults to ADD
    /*
    DPCT1007:912: Migration of CUDNN_POINTWISE_ADD is not supported.
    */
...
class IBlock {
   ...
   protected:
     ...
     */
    virtual dpct::err1
    buildOpGraph(dpct::dnnl::engine_ext &handle, SubGraph const &sub_graph, const std::string &graph_name) {
        (void)handle;
        (void)sub_graph;
        (void)graph_name;
        return 0;
    ...
    calculatePlanExecutionWorkspaceSize() {
        ...
        if (max_workspace_size == -1) {
            logErrorMessage(2000, blockName, "workspace ptr", "workspace ptr", error_message);
            return 2000;
        }
     ...
     */
    ExecutionPlan_v8
    getPlanFromHeuristics(const std::string &graph_name, OperationGraph_v8 &opGraph, dpct::dnnl::engine_ext &handle) {
        getLogger() << "[cudnn_frontend] "
        ...
        for (size_t i = 0; i < filtered_configs.size(); i++) {
            ...
            try {
                ...
                if (i == filtered_configs.size() - 1 && plan.get_status() != 0) {
                    logErrorMessage(
                        5000, blockName, "execution plan descriptor", graph_name, error_message, plan.get_error());
                    return plan;

                } else if (plan.get_status() != 0) {
                    continue;
                }
                getLogger() << "[cudnn_frontend] "
   ...
   public:
     ...
     */
    dpct::err1
    build(dpct::dnnl::engine_ext handle) {
        getLogger() << "[cudnn_frontend]"
    ...
    execute(dpct::dnnl::engine_ext &handle) {
        ...
        while (!copy_sub_graph_queue.empty()) {
            ...
            dpct::err1 status = executePlan(handle, plan_name);
            if (status != 0) {
                getLogger() << "[cudnn_frontend] " << error_message << std::endl;
                return status;
            }
     ...
     */
    dpct::err1
    executePlan(dpct::dnnl::engine_ext &handle, const std::string &plan) {
        getLogger() << "[cudnn_frontend] "
        ...
        dpct::err1 status = executePlanOnce(handle, execution_plan, variant_pack, plan);
     ...
     */
    dpct::err1
    executePlanAndLogTime(dpct::dnnl::engine_ext handle,
    ...
    dpct::device_ext &dev_ct1 = dpct::get_current_device();
        ...
        // Warm-up run
        /*
        DPCT1007:913: Migration of cudnnBackendExecute is not supported.
        */
        ...
        for (int i = 0; i < maxIterCount; i++) {
            ...
            /*
            DPCT1007:914: Migration of cudnnBackendExecute is not supported.
            */
    ...
    dpct::err1
    executePlanOnce(dpct::dnnl::engine_ext handle,
                    ExecutionPlan_v8 &plan,
                    const VariantPack_v8 &variantPack,
                    const std::string &planName) {
        (void)planName;
        /*
        DPCT1007:915: Migration of cudnnBackendExecute is not supported.
        */
        cudnnBackendExecute(handle, plan.get_raw_desc(), variantPack.get_raw_desc());
        getLogger() << "[cudnn_frontend] Plan tag " << plan.getTag() << std::endl;
