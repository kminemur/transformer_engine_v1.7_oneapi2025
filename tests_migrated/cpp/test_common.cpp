...
Tensor::Tensor(const NVTEShape &shape, const DType type) {
  ...
  sycl::queue &q_ct1 = dev_ct1.in_order_queue();
    ...
    if (total_size != 0) {
        dptr = (void *)sycl::malloc_device(total_size, q_ct1); // NOLINT(*)
        q_ct1.memset(dptr, 0, total_size).wait();
    ...
    if (isFp8Type(type)) {
      amax = sycl::malloc_device<float>(1, q_ct1); // NOLINT(*)
      q_ct1.memset(amax, 0, sizeof(float)).wait();
      scale = sycl::malloc_device<float>(1, q_ct1); // NOLINT(*)
      q_ct1.memset(scale, 0, sizeof(float)).wait();
      scale_inv = sycl::malloc_device<float>(1, q_ct1); // NOLINT(*)
      q_ct1.memset(scale_inv, 0, sizeof(float)).wait();
